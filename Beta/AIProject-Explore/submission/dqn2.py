...
class DeepQAgent:
    ...
    def replay(self, batch_size, agent2):
        ...
        for state, action, reward, next_state, done in minibatch:
            target = reward
            target_t = agent2.model.predict(next_state.get_input_layer())
            target_f = self.model.predict(state.get_input_layer())
            if not done:
                target = self.learning_rate * (reward + self.gamma * target_t[0][action] - target_f[0][action])  # Returns q-values
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for time in range(500):
            ...
            if len(agent1.memory) > batch_size and len(agent2.memory) > batch_size:
                if r < 0.5:
        ...
        print("episode: {}/{}, score: {} ,Recent Average: {}, agent: {}".format(e, EPISODES, total_reward, av, 1 if r<0.5 else 2))
        eVSs.append((e+1,av))
        if e % 20 == 0:
